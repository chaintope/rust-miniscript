...
use tapyrus::hashes::{hash160, sha256d, Hash};
...
pub enum InputError {
    ...
    SecpErr(tapyrus::secp256k1::Error),
    /// Key errors
    KeyErr(tapyrus::key::Error),
...
#[doc(hidden)]
impl From<tapyrus::secp256k1::Error> for InputError {
    fn from(e: tapyrus::secp256k1::Error) -> InputError { InputError::SecpErr(e) }
}

#[doc(hidden)]
impl From<tapyrus::key::Error> for InputError {
    fn from(e: tapyrus::key::Error) -> InputError { InputError::KeyErr(e) }
}
...
impl<'psbt, Pk: MiniscriptKey + ToPublicKey> Satisfier<Pk> for PsbtInputSatisfier<'psbt> {
    ...
    fn lookup_raw_pkh_pk(&self, pkh: &hash160::Hash) -> Option<tapyrus::PublicKey> {
        self.psbt.inputs[self.index]
            ...
            .map(|(pubkey, _)| tapyrus::PublicKey::new(*pubkey))
    }
    ...
    ) -> Option<(tapyrus::secp256k1::XOnlyPublicKey, tapyrus::taproot::Signature)> {
        self.psbt.inputs[self.index]
            .tap_script_sigs
            .iter()
            .find(|&((pubkey, lh), _sig)| {
    ...
    fn lookup_ecdsa_sig(&self, pk: &Pk) -> Option<tapyrus::ecdsa::Signature> {
        self.psbt.inputs[self.index]
            .partial_sigs
...
pub trait PsbtExt {
    ...
    fn extract<C: secp256k1::Verification>(
        &self,
        secp: &Secp256k1<C>,
    ) -> Result<tapyrus::Transaction, Error>;
    ...
    /// the [`sighash::TapSighashType::Default`](tapyrus::sighash::TapSighashType::Default) is chosen
    /// for for taproot spends, otherwise [`EcdsaSighashType::All`](tapyrus::sighash::EcdsaSighashType::All) is chosen.
    ...
    ///
    /// [`SighashCache`]: tapyrus::sighash::SighashCache
    fn sighash_msg<T: Borrow<tapyrus::Transaction>>(
        &self,
        idx: usize,
...
impl PsbtExt for Psbt {
    ...
    fn sighash_msg<T: Borrow<tapyrus::Transaction>>(
        ...
        let prevouts = tapyrus::sighash::Prevouts::All(&prevouts);
        let inp_spk =
...
pub trait PsbtInputExt {
    ...
    fn update_with_descriptor_unchecked(
        &mut self,
        descriptor: &Descriptor<DefiniteDescriptorKey>,
    ) -> Result<Descriptor<tapyrus::PublicKey>, descriptor::ConversionError>;
}
...
impl Translator<DefiniteDescriptorKey, tapyrus::PublicKey, descriptor::ConversionError>
    for KeySourceLookUp
{
    fn pk(
        &mut self,
        xpk: &DefiniteDescriptorKey,
    ) -> Result<tapyrus::PublicKey, descriptor::ConversionError> {
    ...
    translate_hash_clone!(DescriptorPublicKey, tapyrus::PublicKey, descriptor::ConversionError);
}
...
mod tests {
    ...
    use tapyrus::bip32::{DerivationPath, Xpub};
    ...
    use tapyrus::{absolute, Amount, OutPoint, TxIn, TxOut};
    ...
    fn test_extract_bip174() {
        let psbt = tapyrus::Psbt::deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
        let secp = Secp256k1::verification_only();
        let tx = psbt.extract(&secp).unwrap();
        let expected: tapyrus::Transaction = deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
    ...
    fn test_update_item_non_tr_multi() {
...

        {
            ...
            let derived = Descriptor::<tapyrus::PublicKey>::from_str(&derived).unwrap();
    ...
    fn test_update_input_checks() {
        ...
        let mut non_witness_utxo = tapyrus::Transaction {
            version: transaction::Version::ONE,
            lock_time: absolute::LockTime::ZERO,
            input: vec![],
            output: vec![TxOut {
                value: Amount::from_tap(1_000),
