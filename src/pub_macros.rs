...
/// ```rust
/// use miniscript::{tapyrus::PublicKey, policy::concrete::Policy, Translator, hash256};
/// use std::str::FromStr;
/// use miniscript::translate_hash_fail;
/// use std::collections::HashMap;
/// use miniscript::tapyrus::hashes::{sha256, hash160, ripemd160};
...
///
/// // Information to translator abstract String type keys to concrete tapyrus::PublicKey.
/// // In practice, wallets would map from String key names to BIP32 keys
/// struct StrPkTranslator {
///     pk_map: HashMap<String, tapyrus::PublicKey>
/// }
///
/// // If we also wanted to provide mapping of other associated types(sha256, older etc),
/// // we would use the general Translator Trait.
/// impl Translator<String, tapyrus::PublicKey, ()> for StrPkTranslator {
///     // Provides the translation public keys P -> Q
///     fn pk(&mut self, pk: &String) -> Result<tapyrus::PublicKey, ()> {
///         self.pk_map.get(pk).copied().ok_or(()) // Dummy Err
///     }
///
///     // Fail for hash types
///     translate_hash_fail!(String, tapyrus::PublicKey, ());
/// }
///
/// let mut pk_map = HashMap::new();
/// pk_map.insert(String::from("alice_key"), tapyrus::PublicKey::from_str(alice_key).unwrap());
/// pk_map.insert(String::from("bob_key"), tapyrus::PublicKey::from_str(bob_key).unwrap());
